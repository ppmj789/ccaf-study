...
# 가이드 단어: 가이드 대역본의 단어 풀이(scripts/guide_words.py 로 뽑음). 가이드에 나오는 순서 그대로.
def guide_part(sec):
    m = re.match(r"Task Statement (\d)\.(\d)", sec)
    if m:
        return f"도메인 {m.group(1)}", f"Task {m.group(1)}.{m.group(2)}"
    if sec.startswith("시나리오"):
        return "5장 시나리오", sec.split(":")[0]
    m = re.match(r"(\d+)\.", sec)
    n = int(m.group(1)) if m else 0
    label = sec.split(". ", 1)[1] if m else "머리말"
    if n <= 5:
        return "1~5장 개요", label
    if n <= 8:
...
if os.path.exists(gpath):
    for x in json.load(open(gpath, encoding="utf-8")):
        part, where = guide_part(x["sec"])
        gwords.append({"word": x["word"], "ko": x["ko"], "pos": "", "part": part, "where": where,
                       "order": x["order"], "n": x["n"], "examples": []})
...
data = {"site": site, "domains": DOMAINS, "scenarios": SCENARIOS, "cards": cards, "words": wordlist, "gwords": gwords, "guide": guide,
