...
- `words/easy.json` — 단어장에서 숨길 쉬운 단어 목록(소문자). 사용자 기준 "토익 700 수준 이상만"(2026-09-18): 토익 700 이하 일반어, 개발자에게 익숙한 외래어·기술 식별자(interface, retry, frontmatter 등), 쉬운 구동사·숙어(at least, rather than 등), 같은 단어의 변형 중복을 넣는다. 흔한 단어라도 시험에서 특별한 뜻으로 쓰이면(sound=타당한, address=다루다, novel=처음 보는, conservative, surface(v.), contract) 남긴다. build.py 가 문제 단어·가이드 단어 둘 다에서 뺀다. 새 단어를 추가할 때도 이 기준 이상만 넣는다.
